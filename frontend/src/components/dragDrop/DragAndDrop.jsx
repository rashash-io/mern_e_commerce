 ...
 function isValidFileType(file) {
  ...
  const handleFileChange =  (event) => {
    ...
    if (selectedFiles && selectedFiles.length > 0 ) {
      ...
      getBase64(selectedFiles, (result)=>{
  ...
  const handleDrop = async (event) => {
    ...
    if ((droppedFiles.length > 0) &&( isValidFileType(droppedFiles[0]))) {
      ...
      const newFiles = Array.from(base64);
...
export default DragAndDrop;
   ...
   function getBase64  (file, callback) {
    ...
    reader.onload =  ()=>{
      ...
      let img = {...file, base64: (reader.result)}
      callback(img)
    }
    reader.onerror = (error)=>{console.log("Error converting to base64 ==>", error)}
  
    // return new Promise((resolve, reject) => {
    //   fileReader.onload = function (event) {
    //     resolve(event.target.result);
    //     reject((reason) =>{console.log("BASE64 Reject reaseon is: ", reason)})
    //   };
    // });
  }
